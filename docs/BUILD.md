...
## Сборка в облаке (GitHub Actions) — ничего не нужно на компьютере

В репозитории есть workflow **`.github/workflows/release.yml`**:

- **вручную**: GitHub → Actions → **release-builds** → Run workflow;
- **автоматически**: при пуше тега `v*`.

Он собирает: macOS `.app` (universal) + Windows `.exe` (на macOS-раннере) и
Linux-бинарь с boot-проверкой. Результат — в артефактах прогона, а по тегу —
ещё и прикрепляется к Release. Валидация данных идёт отдельной джобой.

Локальные версии Godot/шаблонов при этом не нужны вообще.
